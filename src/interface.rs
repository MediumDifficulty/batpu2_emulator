...
use bitvec::{array::BitArray, order::{Lsb0, Msb0}};
...
pub extern "C" fn on_mem_read(mem: &mut [u8; 256], addr: usize) {
    match addr {
        244 => { // Load Pixel at (Pixel X, Pixel Y)
            ...
            mem[addr] = pixel_buffer[pixel_y][pixel_x] as u8;
        }
        ...
        255 => { // Load controller info
            ...
            mem[addr] = info.0.data[0];
        }
        _ => {}
    }
}

pub extern "C" fn on_mem_write(mem: &[u8; 256], addr: usize) {
    match addr {
...
fn get_pixel_coords(mem: &[u8; 256]) -> (usize, usize) {
    let pixel_x = (mem[240] & 0b11111) as usize;
    let pixel_y = (mem[241] & 0b11111) as usize;
